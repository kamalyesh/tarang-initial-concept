...
function Bar(_ref) {
  ...
  var updateSvg = function updateSvg(frequencies) {
    ...
    if (svgRef.current && frequencies.length) {
      svgRef.current.selectAll('rect').data(frequencies).enter().append('rect').attr('fill', function (d) {
        return "#f00";
      }).attr('width', width / frequencies.length - barPadding).attr('x', function (d, i) {
        return i * (width / frequencies.length);
      });
      svgRef.current.selectAll('rect').data(frequencies).attr('y', function (d, i) {
        // console.log({ yd: d })
  ...
  var createSvg = function createSvg(frequencies) {
    ...
    if (!d3) console.warn("d3 is not found. Tarang may not behave as expected.");else {
      ...
      if (!svgRef.current) {
        svgRef.current = d3.select('#' + canvasId).append('svg').attr('height', height).attr('width', width).attr('class', 'my-1').attr('style', coverArtUrl ? "background: url(".concat(coverArtUrl, ");") : 'background: liniear-gradient(to bottom, rgba(245, 246, 252, 0.52), rgba(117, 118, 124, 0.73))').attr('id', "bar_" + canvasId + "_" + (0, _getNextId.default)());
      }
